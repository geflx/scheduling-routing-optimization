...
int main(){
    ...
    for(contArquivo=1; contArquivo<=10; contArquivo++){

        cout<<"****** Executing!!  "<<contArquivo<<"/10 times.******\n";
        int contInstancia = 1;

        ifstream in(fileName);

        string sheetObjFunction = to_string(contArquivo) +"_OBJ_FUNCTION_" +fileName;
        ofstream sheetObjFunctionFile( sheetObjFunction );

        string detailedResult = to_string(contArquivo) +"_DETAILED_RESULT_" +fileName;
        ofstream detailedResultFile( detailedResult );

        string timeResult = to_string(contArquivo) +"_TIME_" +fileName;
