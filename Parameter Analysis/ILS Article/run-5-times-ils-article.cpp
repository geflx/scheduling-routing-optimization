...
int main(){
    ...
    for(contArquivo=1; contArquivo<=5; contArquivo++){
        ...
        string sheetObjFunction = to_string(contArquivo) +"_FINAL_ILS2_OBJ_F_" +fileName;
        ofstream sheetObjFunctionFile( sheetObjFunction );

        string detailedResult = to_string(contArquivo) +"_FINAL_ILS2_DETAIL_" +fileName;
        ofstream detailedResultFile( detailedResult );

        string timeResult = to_string(contArquivo) +"_FINAL_ILS2_TIME_" +fileName;
            ...
            for(int paramMaxIter:maxIter){
                for(int paramMaxIterIls:maxIterIls){
                    for(int paramNumPerturb: numPerturb){
                        ...
                        time_t time2;
                        time(&time2);
                        pair<double,vector<data>> ils2 = ils_rvnd_custom( njobs, ncars, w, P, t, F, d, Q, s, paramMaxIter, paramMaxIterIls, paramNumPerturb);
