...
int main(){
    ...
    for(contArquivo=1; contArquivo<=10; contArquivo++){
        ...
        string sheetObjFunction = to_string(contArquivo) +"_GA_OBJ_FUNCTION_" +fileName;
        ofstream sheetObjFunctionFile( sheetObjFunction );

        string detailedResult = to_string(contArquivo) +"_GA_DETAILED_RESULT_" +fileName;
        ofstream detailedResultFile( detailedResult );

        string timeResult = to_string(contArquivo) +"_GA_TIME_" +fileName;
