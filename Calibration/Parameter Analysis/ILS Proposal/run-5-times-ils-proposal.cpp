...
int main(){
    ...
    for(contArquivo=1; contArquivo<=5; contArquivo++){

        cout<<"****** Executing!!  "<<contArquivo<<"/5 times.******\n";
        int contInstancia = 1;

        ifstream in(fileName);

        string sheetObjFunction = to_string(contArquivo) +"_FINAL_ILS_PROP_OBJ_F_" +fileName;
        ofstream sheetObjFunctionFile( sheetObjFunction );

        string detailedResult = to_string(contArquivo) +"_FINAL_ILS_PROP_DETAIL_" +fileName;
        ofstream detailedResultFile( detailedResult );

        string timeResult = to_string(contArquivo) +"_FINAL_ILS_PROP_TIME_" +fileName;
        ...
        while(in>>numInstancia){
            ...
            vector<int> maxIter{8};
            vector<int> maxIterIls{100};
            vector<int> numPerturb{5};


            cout<<"Executing instance "<<contInstancia++<<"/180 and printing into files.\n";
