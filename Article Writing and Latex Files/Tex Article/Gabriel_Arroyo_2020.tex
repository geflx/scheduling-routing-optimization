...
Nesta Seção são descritos estudos prévios realizados sobre os problemas de Agendamento de Produção, Roteamentos de Veículos e a combinação destes dois.
...
Por se tratar de uma variação do problema ainda não abordada na literatura, a criação de instâncias para o problema tornou-se necessária. As instâncias foram distruibuídas em dois grupos, pequenas e grandes. O primeiro varia entre oito e vinte tarefas, de três à seis veículos, e o segundo entre trinta e cem tarefas, de cinco à doze veículos.

Seja uma tarefa $i$ $\in$ $N$, o tempo de processamento $P_{i}$ de uma tarefa $i$ possui um intervalo limitado, este é definido como:
...
O seu tamanho $s_i$  é correlacionado ao tempo de processamento desta tarefa, assim pode assumir valores entre um e $P$:
...
A peso de penalidade $w_i$ é determinado de forma aleatória em um intervalo. Este peso é explicitado na função objetivo:
\begin{equation}
\label{ins-w}
 w_{i} \in rand[1.0, 5.0]
\end{equation}
 The capacity (Q) of each vehicle $k$ is given in two different ways, depending on the number of jobs in each group of instances:\\
 
    $Q_{k} = max_{i=1}^N\{s_{i}\} + \tau$, where  $\tau$ is defined by:\\
    \begin{itemize}
    \item In \textbf{small} instances: $random[{\frac{\sum_{i=1}^N s_{i}}{K}},\mu*{\frac{\sum_{i=1}^N s_{i}}{K}}]$;\\ 
    \item In \textbf{larger} instances: $random[{\frac{\sum_{i=1}^K s_{i}}{K}},\mu*{\frac{\sum_{i=1}^N s_{i}}{K}}]$;\\ 
\end{itemize}

The cost $F_{k}$ of a vehicle $k$ is assumed to depend on its capacity:\\
    $F_{k} = N*Q_{k}$; \\
    
To determine the travel time $t_{ij}$, first we have to generate the points of each customer and origin.
The machine's coordinates are fixed in an  interval, with its components being:\\ $(x_{0},y_{0}) \sim  x,y \in [400,700]$\\
To generate the customers' coordinates, each one has to accomplish the following constraints:
\begin{itemize}
    \item $(x_{i},y_{i}) \geq (0,0); \forall i=1,...,N $;
    \item $distance(origin, (x_i,y_i)) \leq (\rho * K); \forall i=1,...,N $;
\end{itemize}
The distance type between points is euclidian.\\

Finally, we have the following parameters to generate the instances:\\
 $\delta \in \{0.5, 1.0, 1.5, 2.0, 2.5\};$\\
$ \mu \in \{1, 1.5, 2.0\}$ in \textbf{small} instances and in \textbf{larger} ones $ \mu \in \{1.5, 2.0, 2.5\};$\\
 
For \textbf{small} instances, the numbers of jobs and vehicles used were:\\
 Number of jobs: $N \in \{8,10,15,20\}$;\\
 Number of vehicles: $K \in \{3,4,5, 6\}$\\ 

Now, for \textbf{larger} instances, the following combination was used:\\
 Number of jobs: $N \in \{30,50,80,100\}$;\\
 Number of vehicles: $K \in \{5,8,10,12\}$\\ 
...
\begin{figure}[ht]
	\centering
    \includegraphics[width=1\textwidth]{Images/SOL_REP} 
