...
pair<double, vector<data>> ILS_RVND_IG(int N, int K, const vector<double>& w, const vector<int>& P, const vector<vector<int> >& t,
    const vector<int>& F, const vector<int>& d, const vector<int>& Q, const vector<int>& s, int maxIter, int maxIterIls, int perturbSize, double igPercentage)
{
	// Get greedy rule solutions.
    vector<vector<data>> feasibleSolutions = getGreedyRuleSolutions(N, K, w, P, t, F, d, Q, s);
    ...
    while (true) {
        ...
        for(int j = 0; j < popSize - 1; j++){
            ...
            if (son1.first && son2.first) {
                ...
                if(son1_Obj <= son2_Obj)
                    newPop[j] = son1.second;
                else
                    newPop[j] = son2.second;
            }
            else if (son1.first) {
                newPop[j] = son1.second;
            }
