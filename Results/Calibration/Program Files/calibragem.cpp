...
void printConfigToFile( ofstream &file, double fObj, const char* message, const vector<data> &config, const vector<int> &carCap, const vector<int> &jobSize,
                  int njobs, int ncars, const vector<int> &processTime, const vector<vector<int>> &time, const vector<int> &dueDate,
                  const vector<double> &weight, const vector<int> &carPrices){
    ...
    file<<setw(numJobsFixo*3)<<procOrder;
    int j=0;
    ...
    file<<setw(numJobsFixo*(5.5))<<deliveryOrder<<"\n\n";
}
...
vector<data> perturb( vector<data> &solution, const vector<int> &capacities, const vector<int> &jobSize, int njobs, int ncars, int numPerturb){

    int mode[3] = { 1,2 };
    // double intensity[11]= { 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5};
     double sizePerturb = numPerturb;
...
pair<double, vector<data>> ils_rvnd(int njobs, int ncars, const vector<double> &w,const vector<int> &P, const vector<vector<int>> &t,
    const vector<int> &F , const vector<int> &d, const vector<int> &Q, const vector<int> &s, int maxIter, int maxIterIls, int numPerturb){
    ...
    for(int a=0; a<maxIter; a++){
        ...
        for(int b=0; b< maxIterIls; b++){
            ...
            if( callRvnd.first < bestResult){
             ...
             solution = perturb( solution, Q, s, njobs, ncars, numPerturb);
        }
    }
...
pair<double, vector<data>> ils_rvnd_custom(int njobs, int ncars, const vector<double> &w,const vector<int> &P, const vector<vector<int>> &t,
    ...
    for(int a=0; a<maxIter; a++){
        ...
        for(int b=0; b< maxIterIls; b++){
            ...
            //Danger !!!!!
            //REMOVED TO WORK ONLY IN ILS_RVND
...
int main(){
    ...
    string sheetObjFunction = "OBJ_FUNCTION_" +fileName;
    ofstream sheetObjFunctionFile( sheetObjFunction );

    string detailedResult = "DETAILED_RESULT_" +fileName;
    ofstream detailedResultFile( detailedResult );

    string timeResult = "TIME_" +fileName;
    ofstream timeResultFile( timeResult );

	sheetObjFunctionFile<<"x 5-50-2 5-50-5 5-50-8 5-50-10 5-100-2 5-100-5 5-100-8 5-100-10 8-50-2 8-50-5 8-50-8 8-50-10 8-100-2 8-100-5 8-100-8 8-100-10 10-50-2 10-50-5 10-50-8 10-50-10 10-100-2 10-100-5 10-100-8 10-100-10\n";
	detailedResultFile<<"x 5-50-2 5-50-5 5-50-8 5-50-10 5-100-2 5-100-5 5-100-8 5-100-10 8-50-2 8-50-5 8-50-8 8-50-10 8-100-2 8-100-5 8-100-8 8-100-10 10-50-2 10-50-5 10-50-8 10-50-10 10-100-2 10-100-5 10-100-8 10-100-10\n";
	timeResultFile<<"x 5-50-2 5-50-5 5-50-8 5-50-10 5-100-2 5-100-5 5-100-8 5-100-10 8-50-2 8-50-5 8-50-8 8-50-10 8-100-2 8-100-5 8-100-8 8-100-10 10-50-2 10-50-5 10-50-8 10-50-10 10-100-2 10-100-5 10-100-8 10-100-10\n";

    while(in>>numInstancia){

        sheetObjFunctionFile<<contInstancia<<" ";
        detailedResultFile<<contInstancia<<" ";
        timeResultFile<<contInstancia<<" ";
        ...
        detailedResultFile<<instancia<<endl;
        ...
        //This will not let cout print in scientif notation:   (to do scientif just do cout<<scientific;)
        cout<<fixed;
        ...
        vector<int> maxIter{5,8,10};
        vector<int> maxIterIls{50,100};
        vector<int> numPerturb{2, 5, 8, 10};


        cout<<"Executing instance "<<contInstancia++<<"/60 and printing into files.\n";
        int contParameterInstance = 1;
        for(int paramMaxIter:maxIter){
            for(int paramMaxIterIls:maxIterIls){
                for(int paramNumPerturb: numPerturb){

                    time_t time2;
                    time(&time2);
                    pair<double,vector<data>> ils2 = ils_rvnd( njobs, ncars, w, P, t, F, d, Q, s, paramMaxIter, paramMaxIterIls, paramNumPerturb);

                    sheetObjFunctionFile<<ils2.first<<" ";
                    time_t time2end;
                    time(&time2end);
                    double diff2 = difftime(time2end,time2);
                    printConfigToFile( detailedResultFile, ils2.first, " ", ils2.second, Q, s, njobs, ncars, P, t, d, w, F);
    ...
    sheetObjFunctionFile.close();
    detailedResultFile.close();
