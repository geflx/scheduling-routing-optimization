...
int main(){
    ...
    for(contArquivo=1; contArquivo<=10; contArquivo++){
        ...
        string sheetObjFunction = to_string(contArquivo) +"_ILS2_OBJ_FUNCTION_" +fileName;
        ofstream sheetObjFunctionFile( sheetObjFunction );

        string detailedResult = to_string(contArquivo) +"_ILS2_DETAILED_RESULT_" +fileName;
        ofstream detailedResultFile( detailedResult );

        string timeResult = to_string(contArquivo) +"_ILS2_TIME_" +fileName;
