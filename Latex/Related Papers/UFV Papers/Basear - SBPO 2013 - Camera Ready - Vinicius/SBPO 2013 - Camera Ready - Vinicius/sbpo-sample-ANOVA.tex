...
%\vspace{30mm}
 \begin{center}
 \textbf{Vinícius Vilar Jacob, José Elias C. Arroyo, André Gustavo dos Santos} \\
...
\begin{equation} \label{eq:total_tardness}
f =  \quad \sum_{j=1}^{n}T_j
\end{equation}

O problema é denotado por $1|ST_{sd,b}|\sum{T_j}$ seguindo a notação de três campos $\alpha | \beta | \gamma$ introduzida por \cite{Graham1979} e adaptada por \cite{allahverdi}, onde $1$ denota o ambiente com uma única máquina, $ST_{sd,b}$ é a informação do tempo de preparação dependente da sequência dos lotes ou famílias e $\sum{T_j}$ é o critério do atraso total.

Uma vez que o problema de minimização de atraso total em uma máquina sem famílias é NP-Difícil \citep{Du1990}, segue que o problema $1|ST_{sd,b}|\sum{T_j}$ é pelo menos NP-Difícil no senso comum.
...
%Scheduling problems that consider setup times associated with families has received attention in the literature, because it occurs in practical situations. For example, in the production line of colored plastics, there are setup times  when changing from one color to another \cite{potts1992integrating}. In manufacturing of steel pipes, there are setup times to manufacture tubes of different standards or specifications \cite{byong1990single}. In all cases, the setup time is a bottleneck in the production line and must be treated appropriately.

%single machine com setup dependente da sequencia
Problemas de programação da produção que consideram explicitamente tempos de preparação são de grande importância em sistemas de manufatura. Uma extensa revisão de literatura para estes problemas, considerando diferentes ambientes de produção, foi realizado por \cite{allahverdi}.

%The majority of works assume that the setup time is negligible or part of the job processing time. While this assumption simplifies the analysis and/or reflects certain applications, it adversely affects the solution quality of many applications of scheduling that require an explicit treatment of setup times.

%artigos sobre scheduling com setutp dependente/nao dependente da familia

Alguns estudos foram realizados sobre o problema de sequenciameto em uma máquina com tarefas agrupadas em famílias e tempos de preparação independente das famílias ($ST_{si,b}$). Nos trabalhos realizados por \cite{Kacem2006} e \cite{schaller2007scheduling} foram propostos algorítimos \textit{branch-and-bound} e heurísticas para o problema $1|ST_{si,b}|\sum{T_j}$. 
Para a minimização de adiantamentos e atrasos ($\sum{E_j} + \sum{T_j}$), métodos exatos e heurísticos foram desenvolvidos por \cite{Schaller2008}.
Abordagens exatas também foram usadas por \cite{pan1997single} e \cite{MagazineBaker2000} para resolver o problema $1|ST_{si,b}|L_{max}$, onde $L_{max}$ é o \textit{lateness} máximo.

%problema de seqüenciamento com setup time family
No entanto, há poucos artigos para o problema de programação da produção em uma máquina com tempos de preparação dependentes da sequência e das famílias ($1|ST_{sd,b}|*$). \cite{van1998sequencing} abordou o problema $1|ST_{sd,b}|C_{max}$ para minimizar o tempo de conclusão máximo ($C_{max}$). Estes autores propuseram um algoritmo de tempo polinomial. Para o problema de minimização do tempo total de conclusão ($1|ST_{sd,b}|\sum{C_j}$), \cite{karabat2005minimizing} propuseram um algoritmo \textit{branch-and-bound}. No artigo de  \cite{DBLPjournalsjorsJinGSW10} é abordado o problema $1|ST_{sd,b}|L_{max}$, no qual os autores usam uma heurística de busca tabu. %Nele, os autores usam propriedades estruturais e de vizinhança para excluir soluções não promissoras do espaço de busca em uma heurística de busca tabu.


%artigos para o problema 1|st_sb,b|\sum{T_i}
No melhor do nosso conhecimento, o único artigo que aborda o problema $1|ST_{sd,b}|\sum{T_i}$ foi apresentado por \cite{Chantaravarapan_Gupta_Smith_2003}. Os autores propuseram um algoritmo genético hibrido (HGA). O HGA foi mais eficiente em minimizar o atraso total quando comparado com outros algoritmos heurísticos.

Neste artigo, é apresentado um algoritmo heurístico baseado na metaheurística \textit{Iterated Local Search} (ILS)  para obter soluções aproximadas de alta qualidade para o problema $1|ST_{sd,b}|\sum{T_j}$. ILS é uma metaheurística simples, robusta e efetiva que aplica repetidamente uma busca local a soluções obtidas ao perturbar soluções ótimas locais previamente visitadas. Na heurística proposta, a perturbação é aplicada em níveis, isto é, a perturbação é aumentada à medida que a busca local não consegue gerar novas soluções melhores.

O restante deste artigo está organizado como segue: na Seção \ref{sec:metodology} é descrito os métodos usados na heurística ILS proposta. Os experimentos computacionais são apresentados na Seção \ref{sec:experiments}. Os resultados obtidos são analisados estatisticamente na última Seção. Finalmente, na Seção \ref{sec:conclusions}, estão as conclusões do trabalho.
