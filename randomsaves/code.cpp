...
pair<double, vector<data>> VND(bool reuse, int numTrabalhos, int numVeiculos, const vector<double> &w,const vector<int> &P, const vector<vector<int>> &t, 
   const vector<int> &F , const vector<int> &d, const vector<int> &Q, const vector<int> &s, vector<data> &initialConfig){


    //Checking if is needed to generate a new one or if the current solution will be used
    if( !reuse ){
        initialConfig = generateValidRandomConfig(numTrabalhos,numVeiculos,Q,s);
    }
...
bool validConfig( const vector<data> &config, const vector<int> &capacities, const vector<int> &jobSize ){
...
vector<data> perturb ( vector<data> &solution, const vector<int> &capacities, const vector<int> &jobSizes, int numJobs, int numV){

    int mode[3] = { 1, 2, 3};
    double intensity[11]= { 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5};

    double sizePerturb = in danger

}

void ILS(int numTrabalhos, int numVeiculos, const vector<double> &w,const vector<int> &P, const vector<vector<int>> &t, 
    ...
    if( validConfig( atcConfig, Q, s)) validSolutions.push_back( atcConfig );
    if( validConfig( wmddConfig, Q, s)) validSolutions.push_back( wmddConfig);
    if( validConfig( weddConfig, Q, s)) validSolutions.push_back( weddConfig);
    ...
    for(int a=0; a<maxIter; a++){
        ...
        if( contGetSolution < nbValidSolutions ){
            ...
            callVnd = VND( true, numTrabalhos, numVeiculos, w, P, t, F, d , Q, s, solution);
            contGetSolution++;
        }else{
        //Otherwise we generate a random one
            callVnd = VND( false, numTrabalhos, numVeiculos, w, P, t, F, d , Q, s, solution);
         }

        // vector<vehicleLoaded> vehicleOrder = generateVehicleOrder(atcConfig,numVeiculos);
        // vector<int> deliveryTime = calculatingDeliveryTime(atcConfig,startVehicleTime,t,P,vehicleOrder,numTrabalhos);
        // vector<int> jobTardiness = calculatingJobTardiness(deliveryTime,numTrabalhos,d);
 
        // Refreshing the best solution found
        ...
        for(int b=0; b< maxIterIls; b++){
            ...
            solution = perturb( solution, Q, s, numTrabalhos, numVeiculos);
...
int main(){
    ...
    while(in>>numInstancia){
        ...
        */
        ...
        vector<data> solution;
        pair<double, vector<data>> callVnd = VND(false, numTrabalhos, numVeiculos, w, P, t, F, d , Q, s, solution);
